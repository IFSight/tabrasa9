  ...
  Drupal.slickBrowser = Drupal.slickBrowser || {
    ...
    itemContainer: function (i, elm) {
      $('.button', elm).on('mousedown.sbAction', function () {
        $(this).closest('.item-container').addClass('media--loading is-b-loading');
      });
    },
    ...
    loading: function (event) {
      if (!$(event.currentTarget).hasClass('is-active')) {
        $(event.currentTarget).closest('.js-form-managed-file, .form--sb').addClass('media--loading is-b-loading');
      }
    },
     ...
     */
    loaded: function (all) {
      // @todo remove media--loading post blazy:2.3+.
      $('.js-form-managed-file').removeClass('media--loading is-b-loading');
      if (all) {
        $('.media--loading').removeClass('media--loading is-b-loading');
      }
    }
  };
